...
class RGT():
    ...
    def rgt(self, sample):
        ...
        try:
            ...
            self.export_sheets(sample_code, genotype, ad)

            #Automaticly detect allels from counts table and geom table
            output_table[sample_code] = ad.result_summery
            reads_with_no_flanks = file.get_discarded_reads_percentage()
            reads_with_no_repeats = genotype.get_discarded_reads_percentage()
            output_table[sample_code].append(str(round(reads_with_no_flanks, 1)))
            output_table[sample_code].append(str(round(reads_with_no_repeats, 1)))
            ...
            self.color_code_discarded_reads_percntg(color_table, reads_with_no_repeats,sample_code)
    ...
    def color_code_discarded_reads_percntg(self, color_table, prcntg,sample_code):
        if prcntg >= int(self.settings["discarded_reads_flag_percentage"]):
            ...
            this_sample_dict[11] = "red"

    def export_sheets(self, sample_code, genotype, ad):
        ...
        flanking_seq_table = ad.flanking_sequence_table
        ...
        if self.settings["additional_csv_export"]:
            ...
            csv_flanking_seq_table_path = f"{self.output_directory}/FilesSpecificResults/csv_exports/flanking_seq_table/{sample_code}.csv"
